...
public class MusicPlayerBackpackContainer extends MaidMainContainer {
    public static final ContainerType<MusicPlayerBackpackContainer> TYPE = IForgeContainerType.create((windowId, inv, data) -> new MusicPlayerBackpackContainer(windowId, inv, data.readInt()));
    private static final ResourceLocation EMPTY_CD_SLOT = new ResourceLocation(NetMusic.MOD_ID, "slot/music_cd_slot");
    private final IIntArray data;

    public MusicPlayerBackpackContainer(int id, PlayerInventory inventory, int entityId) {
            ...
            for (int x = 0; x < 6; x++) {
                ...
                addSlot(new SlotItemHandler(maid.getMaidInv(), index, 143 + 18 * x, 57 + 18 * y) {
                    ...
                    public Pair<ResourceLocation, ResourceLocation> getNoItemIcon() {
                        return Pair.of(ReloadResourceEvent.BLOCK_ATLAS_TEXTURE, EMPTY_CD_SLOT);
                    }
                });
            }
        }
    ...
    private boolean playMusic() {
        ...
        if (0 <= slotId && slotId < 24) {
            ...
            if (stackInSlot.getItem() == InitItems.MUSIC_CD.get()) {
                ...
                MaidMusicToClientMessage msg = new MaidMusicToClientMessage(this.maid.getId(), info.songUrl, info.songTime, info.songName, info.songId);
