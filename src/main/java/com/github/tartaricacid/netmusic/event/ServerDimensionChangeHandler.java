...
public class ServerDimensionChangeHandler {
    ...
    public static void onPlayerChangeDimension(PlayerEvent.PlayerChangedDimensionEvent event) {
        ...
        world.getServer().execute(() -> {
            // 从缓存获取正在播放的音乐播放器（全服只有一个）
            TileEntityMusicPlayer musicPlayer = TileEntityMusicPlayer.getActiveInstance();

            if (musicPlayer != null && musicPlayer.isPlay() && musicPlayer.getCurrentTime() > 0) {
